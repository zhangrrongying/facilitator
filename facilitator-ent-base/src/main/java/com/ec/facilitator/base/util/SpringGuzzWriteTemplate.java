...
import org.guzz.web.context.spring.TransactionManagerUtils;
 ...
 */
public class SpringGuzzWriteTemplate extends AbstractWriteTemplate implements WriteTemplate, InitializingBean {
	 ...
	 */
	public SpringGuzzWriteTemplate() {
	}
	 ...
	 * @throws Exception 
	 */
	public SpringGuzzWriteTemplate(TransactionManager transactionManager) throws Exception {
	...
	protected <T> T doExecute(GuzzWriteCallback<T> action, boolean enforceNativeSession) throws DataAccessException {
		...
		//如果在没有使用spring transaction aop标记的时候，我们需要手动生成一个session，并在处理完后关闭它
		//避免connection leak
		boolean createNewSession = false;
		if (session == null) {
			createNewSession = true;
			session = transactionManager.openRWTran(true);
		}
	...
	public ReadonlyTranSession exportReadAPI() {
		...
		return ((WriteTranSessionImpl) session).exportReadAPI() ;
	}
